...
/**
 * Builds an active-alarm incident spec from normalized alarm identity fields.
 * Shared by the report-parsed path (`extractActiveAlarmSpecs`) and the pure
 * SNS mapper (`alarmSpecFromSns`) so push-born and report-born alarms produce
 * byte-identical specs — required for the shared `specDedupeKey` to converge.
 */
export function buildActiveAlarmSpec(params: {
  alarmName: string;
  service: string;
  ecsService?: string;
}): MandatoryIncidentSpec {
  const { alarmName, service, ecsService } = params;
...
export function extractActiveAlarmSpecs(report: string): MandatoryIncidentSpec[] {
  return splitServiceSections(report).flatMap(({ service, body }) => {
    ...
    return alarms.map((alarmName) =>
      buildActiveAlarmSpec({ alarmName, service, ...(ecsService ? { ecsService } : {}) })
    );
  });
}
